...
# =============================================================================
# ABA 4: CONFIGURAÇÃO DO BROKER MQTT
# (campos de texto gravados em mqtt_config.txt, lido posteriormente pelo
#  N2_N3 para estabelecer a conexão com o broker MQTT escolhido)
# =============================================================================
aba_mqtt = Frame(notebook, bg="#F0F0F0")
notebook.add(aba_mqtt, text="  🔌 Broker MQTT  ")

reg_mqtt = Frame(master=aba_mqtt, borderwidth=1, relief='sunken', bg="#F0F0F0")
reg_mqtt.place(x=10, y=10, width=520, height=530)

Label(reg_mqtt, font=("Arial", 14, "bold"), text="Configuração do Broker MQTT",
      padx=5, pady=5, bg="#F0F0F0").pack(side=TOP, anchor="n")

# BROKER
Label(reg_mqtt, text="Broker MQTT", font=("Arial", 12),
      bg="#F0F0F0").place(x=20, y=45)
Label(reg_mqtt, text="ex: test.mosquitto.org ou broker.hivemq.com", font=("Arial", 8),
      bg="#F0F0F0").place(x=20, y=65)
valor_mqtt_broker = Entry(reg_mqtt, width=30, font=("Arial", 12))
valor_mqtt_broker.place(x=260, y=45)
valor_mqtt_broker.insert(0, "test.mosquitto.org")

# PORTA_MQTT
Label(reg_mqtt, text="Porta MQTT", font=("Arial", 12),
      bg="#F0F0F0").place(x=20, y=100)
Label(reg_mqtt, text="ex: 1883 ou 8883", font=("Arial", 8),
      bg="#F0F0F0").place(x=20, y=120)
valor_mqtt_porta = Entry(reg_mqtt, width=30, font=("Arial", 12))
valor_mqtt_porta.place(x=260, y=100)
valor_mqtt_porta.insert(0, "1883")

# CLIENT_ID
Label(reg_mqtt, text="Client ID", font=("Arial", 12),
      bg="#F0F0F0").place(x=20, y=155)
Label(reg_mqtt, text="NA => N2_N3 ignora este campo", font=("Arial", 8),
      bg="#F0F0F0").place(x=20, y=175)
valor_mqtt_client_id = Entry(reg_mqtt, width=30, font=("Arial", 12))
valor_mqtt_client_id.place(x=260, y=155)
valor_mqtt_client_id.insert(0, "NA")

# MQTT_USER
Label(reg_mqtt, text="Usuário MQTT", font=("Arial", 12),
      bg="#F0F0F0").place(x=20, y=210)
Label(reg_mqtt, text="NA => N2_N3 ignora este campo", font=("Arial", 8),
      bg="#F0F0F0").place(x=20, y=230)
valor_mqtt_user = Entry(reg_mqtt, width=30, font=("Arial", 12))
valor_mqtt_user.place(x=260, y=210)
valor_mqtt_user.insert(0, "NA")

# MQTT_PASSWORD
Label(reg_mqtt, text="Senha MQTT", font=("Arial", 12),
      bg="#F0F0F0").place(x=20, y=265)
Label(reg_mqtt, text="NA => N2_N3 ignora este campo", font=("Arial", 8),
      bg="#F0F0F0").place(x=20, y=285)
valor_mqtt_password = Entry(reg_mqtt, width=30, font=("Arial", 12), show="*")
valor_mqtt_password.place(x=260, y=265)
valor_mqtt_password.insert(0, "NA")

# IDENTIFICADOR (usado para montar TOPIC_DL / TOPIC_UL completos)
Label(reg_mqtt, text="Identificador (Aluno)", font=("Arial", 12),
      bg="#F0F0F0").place(x=20, y=320)
Label(reg_mqtt, text="ex: A2F - usado nos tópicos abaixo", font=("Arial", 8),
      bg="#F0F0F0").place(x=20, y=340)
valor_mqtt_id_aluno = Entry(reg_mqtt, width=30, font=("Arial", 12))
valor_mqtt_id_aluno.place(x=260, y=320)
valor_mqtt_id_aluno.insert(0, "A2F")

# Prévia dos tópicos completos, montados a partir do identificador
label_preview_dl = Label(reg_mqtt, text="TOPIC_DL: --", font=("Consolas", 9),
                          fg="gray30", bg="#F0F0F0", anchor="w")
label_preview_dl.place(x=20, y=365, width=480)

label_preview_ul = Label(reg_mqtt, text="TOPIC_UL: --", font=("Consolas", 9),
                          fg="gray30", bg="#F0F0F0", anchor="w")
label_preview_ul.place(x=20, y=385, width=480)

# MQTT_QOS (caixa de seleção ativado/desativado)
var_mqtt_qos = IntVar(value=1)
chk_mqtt_qos = Checkbutton(reg_mqtt, text="QoS 1 (ativado) / QoS 0 (desativado)",
                            variable=var_mqtt_qos, font=("Arial", 11), bg="#F0F0F0")
chk_mqtt_qos.place(x=20, y=415)

# Status da gravação
status_texto_mqtt = StringVar()
status_texto_mqtt.set("Configuração MQTT ainda não salva")
label_status_mqtt = Label(reg_mqtt, textvariable=status_texto_mqtt,
                           font=("Arial", 9, "bold"), fg="gray30", bg="#F0F0F0")
label_status_mqtt.place(x=20, y=445)


def _montar_topicos_mqtt():
    identificador = valor_mqtt_id_aluno.get().strip()
    topic_dl = "mot_lora_mqtt_" + identificador + "/gateway/downlink"
    topic_ul = "mot_lora_mqtt_" + identificador + "/gateway/uplink"
    return topic_dl, topic_ul


def _atualizar_preview_topicos_mqtt(event=None):
    topic_dl, topic_ul = _montar_topicos_mqtt()
    label_preview_dl.config(text="TOPIC_DL: " + topic_dl)
    label_preview_ul.config(text="TOPIC_UL: " + topic_ul)


valor_mqtt_id_aluno.bind("<KeyRelease>", _atualizar_preview_topicos_mqtt)
_atualizar_preview_topicos_mqtt()


def gravar_config_mqtt():
    broker = valor_mqtt_broker.get().strip()
    porta = valor_mqtt_porta.get().strip()

    # Campos que, quando deixados vazios, caem no padrão NA (N2_N3 os ignora)
    client_id = valor_mqtt_client_id.get().strip() or "NA"
    mqtt_user = valor_mqtt_user.get().strip() or "NA"
    mqtt_password = valor_mqtt_password.get().strip() or "NA"

    topic_dl, topic_ul = _montar_topicos_mqtt()
    qos = "1" if var_mqtt_qos.get() else "0"

    arquivo_txt = os.path.join(dir_nivel4, 'mqtt_config.txt')
    try:
        with open(arquivo_txt, 'w') as s:
            s.write(broker + "\n")
            s.write(porta + "\n")
            s.write(client_id + "\n")
            s.write(mqtt_user + "\n")
            s.write(mqtt_password + "\n")
            s.write(topic_dl + "\n")
            s.write(topic_ul + "\n")
            s.write(qos + "\n")
        status_texto_mqtt.set("Configuração MQTT salva em mqtt_config.txt")
        label_status_mqtt.config(fg="green")
    except Exception as e:
        status_texto_mqtt.set("Erro ao salvar mqtt_config.txt: " + str(e))
        label_status_mqtt.config(fg="red")


btn_salvar_mqtt = Button(reg_mqtt, text="SALVAR CONFIGURAÇÃO MQTT", font=("Arial", 11, "bold"),
                          command=gravar_config_mqtt)
btn_salvar_mqtt.place(x=20, y=470)


# =============================================================================
# ABA 5: CADASTRO DE NÓS SENSORES / GATEWAY LoRa
# Cadastro (Novo / Salvar / Excluir) dos end devices da rede LoRa, gravado
# em end-devices.txt (chave mestre = endereço do nó; cada registro ocupa
# um bloco de CAMPOS_POR_NO linhas: endereço, SF, BW, CR, latitude,
# longitude - novos nós são acrescentados como novos blocos ao final do
# arquivo). Também cadastra o gateway LoRa único da rede em
# gateway_lora.txt (endereço, latitude, longitude), que é sempre
# sobrescrito por completo a cada gravação, já que só existe uma linha de
# cadastro prevista para o gateway.
# =============================================================================
aba_cadastro = Frame(notebook, bg="#F0F0F0")
notebook.add(aba_cadastro, text="  🛰️ Cadastro de Nós  ")

arquivo_end_devices = os.path.join(dir_nivel4, 'end-devices.txt')
arquivo_gateway = os.path.join(dir_nivel4, 'gateway_lora.txt')

CAMPOS_POR_NO = 6  # endereço, SF, BW, CR, latitude, longitude

# --- painel de cadastro de nó sensor (end device) ---
reg_no_sensor = Frame(master=aba_cadastro, borderwidth=1, relief='sunken', bg="#F0F0F0")
reg_no_sensor.place(x=10, y=10, width=520, height=380)

Label(reg_no_sensor, font=("Arial", 14, "bold"), text="Cadastro de Nó Sensor (End Device)",
      padx=5, pady=5, bg="#F0F0F0").pack(side=TOP, anchor="n")

# Endereço (Combobox editável - clicar na seta expande os nós já cadastrados)
Label(reg_no_sensor, text="Endereço do Nó Sensor", font=("Arial", 12),
      bg="#F0F0F0").place(x=20, y=45)
Label(reg_no_sensor, text="chave mestre - clique na seta p/ ver cadastrados", font=("Arial", 8),
      bg="#F0F0F0").place(x=20, y=65)
combo_endereco_no = ttk.Combobox(reg_no_sensor, width=25, font=("Arial", 12), state="normal")
combo_endereco_no.place(x=260, y=45)

# Spreading Factor
Label(reg_no_sensor, text="Spreading Factor (SF)", font=("Arial", 12),
      bg="#F0F0F0").place(x=20, y=100)
Label(reg_no_sensor, text="7 a 12", font=("Arial", 8), bg="#F0F0F0").place(x=30, y=120)
valor_no_sf = Entry(reg_no_sensor, width=10, font=("Arial", 12))
valor_no_sf.place(x=260, y=100)

# Bandwidth
Label(reg_no_sensor, text="Bandwidth (BW)", font=("Arial", 12),
      bg="#F0F0F0").place(x=20, y=135)
Label(reg_no_sensor, text="125, 250, 500 kHz", font=("Arial", 8), bg="#F0F0F0").place(x=30, y=155)
valor_no_bw = Entry(reg_no_sensor, width=10, font=("Arial", 12))
valor_no_bw.place(x=260, y=135)

# Coding Rate
Label(reg_no_sensor, text="Coding Rate (CR)", font=("Arial", 12),
      bg="#F0F0F0").place(x=20, y=170)
Label(reg_no_sensor, text="5 a 8 => 4/5, 4/6, 4/7, 4/8", font=("Arial", 8), bg="#F0F0F0").place(x=30, y=190)
valor_no_cr = Entry(reg_no_sensor, width=10, font=("Arial", 12))
valor_no_cr.place(x=260, y=170)

# Coordenadas GPS
Label(reg_no_sensor, text="Latitude (GPS)", font=("Arial", 12),
      bg="#F0F0F0").place(x=20, y=205)
valor_no_lat = Entry(reg_no_sensor, width=16, font=("Arial", 12))
valor_no_lat.place(x=260, y=205)

Label(reg_no_sensor, text="Longitude (GPS)", font=("Arial", 12),
      bg="#F0F0F0").place(x=20, y=240)
valor_no_longitude = Entry(reg_no_sensor, width=16, font=("Arial", 12))
valor_no_longitude.place(x=260, y=240)

# Status do cadastro
status_texto_no = StringVar()
status_texto_no.set("Nenhum nó sensor selecionado")
label_status_no = Label(reg_no_sensor, textvariable=status_texto_no,
                         font=("Arial", 9, "bold"), fg="gray30", bg="#F0F0F0",
                         wraplength=480, justify="left")
label_status_no.place(x=20, y=280, width=480)


# --- Funções de persistência em end-devices.txt ---

def _ler_registros_end_devices():
    """Lê end-devices.txt e retorna a lista de registros já cadastrados,
    cada um como uma lista de CAMPOS_POR_NO strings, na ordem: endereço,
    SF, BW, CR, latitude, longitude - na mesma ordem em que aparecem no
    arquivo (blocos de CAMPOS_POR_NO linhas por nó sensor)."""
    registros = []
    try:
        with open(arquivo_end_devices, 'r') as f:
            linhas = [linha.rstrip("\n") for linha in f.readlines()]
    except FileNotFoundError:
        return registros
    for i in range(0, len(linhas) - CAMPOS_POR_NO + 1, CAMPOS_POR_NO):
        bloco = linhas[i:i + CAMPOS_POR_NO]
        if bloco[0].strip():
            registros.append(bloco)
    return registros


def _gravar_registros_end_devices(registros):
    """Reescreve end-devices.txt por completo a partir da lista de
    registros (cada nó sensor cadastrado como um novo bloco de linhas)."""
    with open(arquivo_end_devices, 'w') as f:
        for bloco in registros:
            for campo in bloco:
                f.write(str(campo) + "\n")


def _atualizar_combo_enderecos():
    registros = _ler_registros_end_devices()
    combo_endereco_no['values'] = [bloco[0] for bloco in registros]


def limpar_campos_no_sensor():
    combo_endereco_no.set("")
    valor_no_sf.delete(0, END)
    valor_no_bw.delete(0, END)
    valor_no_cr.delete(0, END)
    valor_no_lat.delete(0, END)
    valor_no_longitude.delete(0, END)


def novo_no_sensor():
    """Botão NOVO: limpa os campos para o cadastro de um nó sensor
    ainda não existente em end-devices.txt."""
    limpar_campos_no_sensor()
    valor_no_sf.insert(0, "12")
    valor_no_bw.insert(0, "125")
    valor_no_cr.insert(0, "8")
    status_texto_no.set("Novo cadastro - preencha o endereço e os parâmetros e clique em Salvar")
    label_status_no.config(fg="blue")
    combo_endereco_no.focus_set()


def selecionar_no_sensor(event=None):
    """Ao selecionar, no Combobox, um endereço já cadastrado, abre os
    parâmetros de rede LoRa (SF/BW/CR) e as coordenadas GPS deste end
    device nos campos abaixo."""
    endereco = combo_endereco_no.get().strip()
    for bloco in _ler_registros_end_devices():
        if bloco[0].strip() == endereco:
            valor_no_sf.delete(0, END); valor_no_sf.insert(0, bloco[1])
            valor_no_bw.delete(0, END); valor_no_bw.insert(0, bloco[2])
            valor_no_cr.delete(0, END); valor_no_cr.insert(0, bloco[3])
            valor_no_lat.delete(0, END); valor_no_lat.insert(0, bloco[4])
            valor_no_longitude.delete(0, END); valor_no_longitude.insert(0, bloco[5])
            status_texto_no.set("Nó sensor '" + endereco + "' carregado de end-devices.txt")
            label_status_no.config(fg="gray30")
            return
    status_texto_no.set("Endereço não encontrado no cadastro")
    label_status_no.config(fg="red")


combo_endereco_no.bind("<<ComboboxSelected>>", selecionar_no_sensor)


def salvar_no_sensor():
    """Botão SALVAR: grava (cadastra ou atualiza) o nó sensor em
    end-devices.txt, usando o endereço como chave mestre do registro."""
    endereco = combo_endereco_no.get().strip()
    if not endereco:
        status_texto_no.set("Informe o endereço do nó sensor antes de salvar")
        label_status_no.config(fg="red")
        return

    sf = valor_no_sf.get().strip()
    bw = valor_no_bw.get().strip()
    cr = valor_no_cr.get().strip()
    lat = valor_no_lat.get().strip()
    longitude = valor_no_longitude.get().strip()

    if not (sf and bw and cr and lat and longitude):
        status_texto_no.set("Preencha SF, BW, CR e coordenadas GPS antes de salvar")
        label_status_no.config(fg="red")
        return

    novo_bloco = [endereco, sf, bw, cr, lat, longitude]
    registros = _ler_registros_end_devices()

    atualizado = False
    for indice, bloco in enumerate(registros):
        if bloco[0].strip() == endereco:
            registros[indice] = novo_bloco
            atualizado = True
            break
    if not atualizado:
        registros.append(novo_bloco)

    try:
        _gravar_registros_end_devices(registros)
        _atualizar_combo_enderecos()
        combo_endereco_no.set(endereco)
        if atualizado:
            status_texto_no.set("Nó sensor '" + endereco + "' atualizado em end-devices.txt")
        else:
            status_texto_no.set("Nó sensor '" + endereco + "' cadastrado em end-devices.txt")
        label_status_no.config(fg="green")
    except Exception as e:
        status_texto_no.set("Erro ao salvar end-devices.txt: " + str(e))
        label_status_no.config(fg="red")


def excluir_no_sensor():
    """Botão EXCLUIR: remove o registro do nó sensor selecionado (pela
    chave mestre = endereço) de end-devices.txt."""
    endereco = combo_endereco_no.get().strip()
    if not endereco:
        status_texto_no.set("Selecione ou informe o endereço do nó sensor a excluir")
        label_status_no.config(fg="red")
        return

    registros = _ler_registros_end_devices()
    registros_restantes = [bloco for bloco in registros if bloco[0].strip() != endereco]

    if len(registros_restantes) == len(registros):
        status_texto_no.set("Endereço '" + endereco + "' não encontrado no cadastro")
        label_status_no.config(fg="red")
        return

    if not tkMessageBox.askyesno("Excluir Nó Sensor",
                                  "Confirma a exclusão do nó sensor '" + endereco + "' ?"):
        return

    try:
        _gravar_registros_end_devices(registros_restantes)
        _atualizar_combo_enderecos()
        limpar_campos_no_sensor()
        status_texto_no.set("Nó sensor '" + endereco + "' excluído de end-devices.txt")
        label_status_no.config(fg="green")
    except Exception as e:
        status_texto_no.set("Erro ao excluir de end-devices.txt: " + str(e))
        label_status_no.config(fg="red")


btn_novo_no = Button(reg_no_sensor, text="NOVO", font=("Arial", 11, "bold"), width=10,
                      command=novo_no_sensor)
btn_novo_no.place(x=20, y=330)

btn_salvar_no = Button(reg_no_sensor, text="SALVAR", font=("Arial", 11, "bold"), width=10,
                        command=salvar_no_sensor)
btn_salvar_no.place(x=170, y=330)

btn_excluir_no = Button(reg_no_sensor, text="EXCLUIR", font=("Arial", 11, "bold"), width=10,
                         command=excluir_no_sensor)
btn_excluir_no.place(x=320, y=330)

_atualizar_combo_enderecos()


# --- painel de cadastro do gateway LoRa (único na rede) ---
reg_gateway = Frame(master=aba_cadastro, borderwidth=1, relief='sunken', bg="#F0F0F0")
reg_gateway.place(x=550, y=10, width=420, height=250)

Label(reg_gateway, font=("Arial", 14, "bold"), text="Gateway LoRa (único na rede)",
      padx=5, pady=5, bg="#F0F0F0").pack(side=TOP, anchor="n")

Label(reg_gateway, text="Endereço do Gateway", font=("Arial", 12),
      bg="#F0F0F0").place(x=20, y=45)
valor_gw_endereco = Entry(reg_gateway, width=20, font=("Arial", 12))
valor_gw_endereco.place(x=210, y=45)

Label(reg_gateway, text="Latitude (GPS)", font=("Arial", 12),
      bg="#F0F0F0").place(x=20, y=90)
valor_gw_lat = Entry(reg_gateway, width=20, font=("Arial", 12))
valor_gw_lat.place(x=210, y=90)

Label(reg_gateway, text="Longitude (GPS)", font=("Arial", 12),
      bg="#F0F0F0").place(x=20, y=135)
valor_gw_longitude = Entry(reg_gateway, width=20, font=("Arial", 12))
valor_gw_longitude.place(x=210, y=135)

status_texto_gw = StringVar()
status_texto_gw.set("Gateway ainda não carregado")
label_status_gw = Label(reg_gateway, textvariable=status_texto_gw,
                         font=("Arial", 9, "bold"), fg="gray30", bg="#F0F0F0",
                         wraplength=380, justify="left")
label_status_gw.place(x=20, y=170, width=380)


def carregar_gateway():
    """Lê gateway_lora.txt (endereço / latitude / longitude - única linha
    de cadastro prevista, já que só há um gateway na rede) e preenche os
    campos, caso o arquivo já exista."""
    try:
        with open(arquivo_gateway, 'r') as f:
            linhas = [linha.strip() for linha in f.readlines()]
    except FileNotFoundError:
        return
    if len(linhas) >= 3:
        valor_gw_endereco.delete(0, END); valor_gw_endereco.insert(0, linhas[0])
        valor_gw_lat.delete(0, END); valor_gw_lat.insert(0, linhas[1])
        valor_gw_longitude.delete(0, END); valor_gw_longitude.insert(0, linhas[2])
        status_texto_gw.set("Gateway '" + linhas[0] + "' carregado de gateway_lora.txt")
        label_status_gw.config(fg="gray30")


def salvar_gateway():
    """Botão SALVAR GATEWAY: sobrescreve por completo gateway_lora.txt
    com o endereço e as coordenadas GPS atuais (único gateway da rede,
    portanto sempre a mesma e única linha de cadastro é atualizada)."""
    endereco = valor_gw_endereco.get().strip()
    lat = valor_gw_lat.get().strip()
    longitude = valor_gw_longitude.get().strip()

    if not (endereco and lat and longitude):
        status_texto_gw.set("Preencha endereço e coordenadas GPS do gateway antes de salvar")
        label_status_gw.config(fg="red")
        return

    try:
        with open(arquivo_gateway, 'w') as f:
            f.write(endereco + "\n")
            f.write(lat + "\n")
            f.write(longitude + "\n")
        status_texto_gw.set("Gateway '" + endereco + "' salvo em gateway_lora.txt")
        label_status_gw.config(fg="green")
    except Exception as e:
        status_texto_gw.set("Erro ao salvar gateway_lora.txt: " + str(e))
        label_status_gw.config(fg="red")


btn_salvar_gw = Button(reg_gateway, text="SALVAR GATEWAY", font=("Arial", 11, "bold"),
                        command=salvar_gateway)
btn_salvar_gw.place(x=20, y=205)
